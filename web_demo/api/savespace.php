...
if(isset($_REQUEST['userid']) && isset($_REQUEST['data']))
{
    try
    {
        $data = $_REQUEST['data'];
        ...
        if(!isset($data->spaceinfo) || is_null($data->spaceinfo))
        {
            _exit(0, "Missing parameter: space info");
        }

        if(!isset($data->travelplan) || is_null($data->travelplan))
        {
             _exit(0, "Missing parameter: travel plan");
        }

        if(!isset($data->pickupinfo) || is_null($data->pickupinfo))
        {
             _exit(0, "Missing parameter: pick up info");
        }

        if(!isset($data->deliveryinfo) || is_null($data->deliveryinfo))
        {
            _exit(0, "Missing parameter: delivery info");
        }
        ...
        //Space Info
        if(!isset($spaceinfo->maxweight))
            _exit(0, "Not enough parameter: max weight");

        if($spaceinfo->maxweight == 0)
            _exit(0, "Max weight must be bigger than 0");

        if(!isset($spaceinfo->minimumbaseprice))
            _exit(0, "Not enough parameter: minimum base price");

        if($spaceinfo->minimumbaseprice == 0)
            _exit(0, "Minimum base price must be bigger than 0");

        if(!isset($spaceinfo->priceperweight))
            _exit(0, "Not enough parameter: Price");

        if($spaceinfo->priceperweight == 0)
            _exit(0, "Price must be bigger than 0");

        if(!isset($spaceinfo->itemallowed))
            _exit(0, "Not enough parameter: Item allowed");

        if(count($spaceinfo->itemallowed) == 0)
            _exit(0, "Allowed items is empty");
        ...
        if(!isset($pickupinfo->pickupAddresses[0]->city) || $pickupinfo->pickupAddresses[0]->city == "")
            _exit(0, "Pick up location is empty!");

        if(!isset($pickupinfo->latestPickupDate) || $pickupinfo->latestPickupDate == "")
            _exit(0, "Pick up date is invalid!");
        ...
        if(!isset($deliveryinfo->deliveryAddressInfo->city) || $deliveryinfo->deliveryAddressInfo->city == "")
            _exit(0, "Delivery location is empty!");

        if(!isset($deliveryinfo->latestDeliverDate) || $deliveryinfo->latestDeliverDate == "")
            _exit(0, "Delivery date is invalid!");
        ...
        $space->delivery_location = $deliveryLocation;
        ...
        $space->insurance = "1";
        $space->list_flightid = "";
        $space->drop_off_for_vakargo_scooter = $drop_off_for_vakargo_scooter;
        ...
        if($space_id != FALSE)
        {
            ...
            if(isset($itemallowed->others))
            {
                $cs = Space_CategoryDTO::newSpace_CategoryDTO($space_id, "3", 0);
                $result = Space_CategoryDAO::add($cs);
            }
        }
        else
    ...
    {
        _exit(401, $e->getMessage());
    }
}
else
{
    _exit(0, "Not enough parameters");
}
?>